...
function l(e, s) {
    ...
    if (!s) return i;
    const l = n ? c(t[s], n[s]) : t[s],
        r = Object.keys(l).filter((e) => e in i);
    if (r.length > 0)
        throw new Error(
            `[getConfig] Name conflict in "${e}.${String(s)}": keys [${r.join(', ')}] exist in both componentConfig and commonConfig.`,
        );
    return { ...l, ...i };
}
const r = {
...
function d({ highlight: a, assets: i, counts: t, size: o = g.small, className: c = '' }) {
    const { icons: l, videos: d } = i,
        h = a ? 'none' : l.default;
    return e.jsxs('div', {
        className: s(r.base, r[`base__size-${o}`], a && r.base__highlight, c),
        children: [
            a
                ? e.jsx('div', {
                      className: r.media,
                      children: e.jsx(n, { loop: !0, autoplay: !0, className: r.glow, src: d.glow }),
                  })
                : e.jsx('div', { className: r.media, style: { backgroundImage: `url(${h})` } }),
            e.jsx('div', { className: r.count, children: t }),
        ],
    });
}
const h = 'Highlight_c2944c51',
    m = 'Highlight_base__highlight_8b97c5a1';
const u = 'Info_9c6dfc74';
const b = 'Title_1dcff219',
    _ = 'Title_wrapper_5727057f';
function f({ children: a, className: n, ...i }) {
    return e.jsx('div', { ...i, className: s(b, n), children: e.jsx('div', { className: _, children: a }) });
}
((f.Info = function ({ icon: a, className: n }) {
    ...
    (f.Highlight = function ({ text: a, highlight: n, className: i }) {
        return e.jsx('div', { className: s(h, n && m, i), children: a });
    }));
const N = {
...
function v({ counts: s, size: a = g.small, eventName: n, className: t }) {
    const o = (function (e, s) {
        ...
        { guaranteed: c } = l(n);
    return e.jsx(d, { className: t, highlight: s <= c.accent, counts: s, size: a, assets: o });
}
export { v as S, f as T, l as g, g as s };
