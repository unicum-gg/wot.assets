import { r as s, D as e, a2 as a } from './lib.js';
var i = ((s) => (
    (s.Big = 'big'),
    (s.Small = 'small'),
    (s.Mini = 'mini'),
...
const u = 'vehicles',
    ...
    G = (e, a = 's180x135') => {
        const { name: i, isRent: G, icon: M, id: W, value: H } = e;
        switch (i) {
            case u:
                return G
            ...
            case k:
                return `R.images.gui.maps.icons.quests.bonuses.${a}.${M}`;
            case f:
                return `R.images.gui.maps.icons.quests.bonuses.badges.${D(a)}.${M}`;
            case y:
                return `R.images.gui.maps.icons.achievement.${D(a)}.${M}`;
            case T:
            case w:
            ...
            case A:
            case N:
                return s.resolve('images').has(`R.images.gui.maps.icons.quests.bonuses.${a}.${M}_${H}`)
                    ? `R.images.gui.maps.icons.quests.bonuses.${a}.${M}_${H}`
                    : `R.images.gui.maps.icons.quests.bonuses.${a}.${M}`;
            case z:
            ...
            case C:
                return `R.images.gui.maps.icons.collectionItems.${D(a)}.${M}`;
            default:
                return `R.images.gui.maps.icons.quests.bonuses.${a}.${M}`;
        }
    };
function M({ size: a, name: i, special: u }) {
    let n = a;
    return (
        's360x270' === a && (n = 's400x300'),
        i === c
            ? s.resolve('images').readOrEmpty(`customization.rarity.glowWithSign.${a}.${u}`)
            : s.resolve('images').readOrEmpty(`quests.bonuses.${n}.${e(u)}_overlay`)
    );
}
const W = ['small', 'big'],
    H = (s, e) => {
        if (void 0 === e || !W.includes(s)) return null;
        switch (e) {
...
export { i as I, M as a, G as b, H as g };
