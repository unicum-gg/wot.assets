'use strict';
(self.webpackChunkgameface = self.webpackChunkgameface || []).push([
    [86],
    {
        2980: (e, a, t) => {
            (t.r(a), t.d(a, { default: () => ca }));
            var r = t(7363),
                n = t.n(r),
                s = t(8958),
                i = t(295),
                o = t(8526),
                c = t(5521),
                l = t(5834),
                m = t(3403),
                d = t(7030),
                p = t(3597),
                u = t(198),
                _ = t(6328),
                g = t(2914),
                w = t(9744),
                b = t(9210),
                y = t(6486),
                v = t(9480),
                f = t(9605),
                h = t(7672);
            const R = (0, r.createContext)(null),
                E = 'INIT_ROW',
                A = 'CONCURRENT_APPEAR',
                C = 'REWARD',
                x = 'OVERLAY_REWARD',
                N = 'RARE_VIDEO',
                k = 'COMPENSATION',
                O = 'TWITCH',
                T = 'FINISH',
                I = {
                    [E]: { duration: 300 },
                    [A]: { duration: 200 },
                    [C]: { duration: 150 },
                    [x]: { duration: 0, pauseNextSteps: !0 },
                    [N]: { duration: 0, pauseNextSteps: !0 },
                    [O]: { duration: 1e3 },
                    [k]: { duration: 800 },
                    [T]: { name: T, duration: 0, delay: 100 },
                },
                B = (e, a, t) => {
                    switch (e) {
                        case E:
                            return `animation_${a}`;
                        case A:
                            return `animation_${a}_${A}`;
                        default:
                            return `animation_${a}_${t}_${e}`;
                    }
                },
                P = (e, a, t) => Object.assign({}, I[e], { name: B(e, a, t) }),
                S = (e, a) => {
                    const t = [];
                    return (
                        v.u4(
                            e,
                            (e, t, r) => (
                                0 !== r && e.push(P(E, r)),
                                e.push(P(A, r)),
                                v.UI(t, (t, n) => {
                                    const s = (0, h.y)(t),
                                        i = (0, f.C)(a, t.specialAwardName);
                                    (s
                                        ? (i ? e.push(P(x, r, n)) : e.push(P(N, r, n)), e.push(P(C, r, n)))
                                        : 0 !== n && e.push(P(C, r, n)),
                                        t.isCompensation && (e.push(P(O, r, n)), e.push(P(k, r, n))));
                                }),
                                e
                            ),
                            t,
                        ),
                        t.push(I.FINISH),
                        t
                    );
                };
            var V = t(4736),
                D = t(3215),
                H = t(4598),
                G = t(9174),
                W = t(3946),
                Z = t(7366),
                $ = t(9674);
            const K = {
                    images: {
                        iconEmpty: 'entry_point.lootboxEmpty',
                        previewIcon: 'awardViews.previewIcon',
                        compensationIcon: 'awardViews.compensationIcon',
                        loader: 'common.waiting',
                    },
                    videos: {
                        compensationGlow: 'awardViews.compensationGlow',
                        rareGlow: 'awardViews.rareGlow',
                        rare: 'awardViews.raritySimpleAnimations.rare_small',
                        epic: 'awardViews.raritySimpleAnimations.epic_small',
                    },
                    texts: {
                        closeButton: 'common.closeButton',
                        multiplier: 'common.rewards.multiplier',
                        headerTitle: 'multipleRewardView.header.title',
                        headerSubtitle: 'multipleRewardView.header.subTitle',
                        quantityAvailableTitle: 'quantityTitle.boxesAvailable',
                        quantityNoBoxesTitle: 'quantityTitle.noBoxes',
                        checkbox: 'common.footer.checkbox',
                        footerBackButton: 'awardViews.footer.backButton',
                        footerPurchaseButtonText: 'common.getButton.upperCase',
                        footerOpenNextButton: 'multipleRewardView.footer.openNextButton',
                        loader: 'common.loader',
                        rareOverlayButtonContinue: 'rareRewardOverlay.rewardDescription.button.text',
                        rareOverlayTitle: 'rareRewardOverlay.rewardDescription.title.text',
                        rareOverlayStyleTitle: 'rareRewardOverlay.rewardDescription.style.text',
                        rareOverlayStyleDescription: 'rareRewardOverlay.rewardDescription.style.description.text',
                    },
                    sounds: {
                        multiRewardAppear: $.T.multiRewardAppear,
                        compensationAppear: $.T.compensationAppear,
                        rareAnimation: $.T.rareAnimation,
                        epicAnimation: $.T.epicAnimation,
                        open: $.T.open,
                        openRare: $.T.openRare,
                        purchaseHover: $.T.purchaseHover,
                        purchaseClick: $.T.purchaseClick,
                    },
                },
                j = {
                    dynamicVideos: { boxesOpening: 'awardViews.openingBoxVideo' },
                    dynamicTexts: {
                        rewardsPremiumDay: 'common.rewards.premiumDay',
                        boxCategory: 'common.boxCategory.lowerCase',
                    },
                    dynamicImages: { boxesOpening: 'awardViews.openingBox' },
                },
                z = (0, D.q3)()(
                    ({ observableModel: e }) => {
                        const a = {
                                root: e.object(),
                                bonuses: e.array('bonuses'),
                                info: e.primitives([
                                    'eventName',
                                    'isAnimationActive',
                                    'openingCount',
                                    'boxesCount',
                                    'isAwaitingResponse',
                                    'isReopen',
                                ]),
                                activeRareReward: G.LO.box(null),
                            },
                            t = (0, W.Om)(() => (0, Z.jh)(K, a.info.eventName.get()), { equals: H.jv }),
                            r = (0, W.Om)(() => (0, Z.kC)(j, a.info.eventName.get()), { equals: H.jv }),
                            n = (0, W.Om)(() => a.bonuses.get(), { equals: H.jv }),
                            s = (0, W.Om)(() => v.G(n(), (e) => v.G(e, (e) => (0, h.y)(e))), { equals: H.jv }),
                            i = (0, W.Om)(() => v.u4(a.bonuses.get(), (e, a) => (e > a.length ? e : a.length), 0)),
                            o = (0, W.Om)(() => {
                                const e = a.root.get().boxCategory,
                                    n = `${e}_${V.G.Common}`,
                                    s = `${e}_${V.G.Rare}`,
                                    i = r().dynamicImages.boxesOpening.dynOpt(n, V.G.Common),
                                    o = r().dynamicVideos.boxesOpening.dynOpt(n, V.G.Common),
                                    c = r().dynamicImages.boxesOpening.dynOpt(s, V.G.Rare),
                                    l = r().dynamicVideos.boxesOpening.dynOpt(s, V.G.Rare),
                                    m = t().sounds;
                                return {
                                    [y.e.common]: { video: o, image: i, sound: m.open },
                                    [y.e.rare]: { video: l, image: c, sound: m.openRare },
                                };
                            });
                        return Object.assign({}, a, {
                            computes: {
                                rewardsListByBoxes: n,
                                resources: t,
                                dynamicResources: r,
                                maxRewardsInLine: i,
                                hasRareReward: s,
                                multimediaResource: o,
                            },
                        });
                    },
                    ({ externalModel: e, model: a }) => ({
                        goPreview: e.createCallback((e) => e, 'onPreview'),
                        openNext: e.createCallback(() => {
                            const e = a.info,
                                t = e.boxesCount,
                                r = e.openingCount;
                            return { openCount: Math.min(t.get(), r.get()) };
                        }, 'onOpen'),
                        goBack: e.createCallbackNoArgs('onGoBack'),
                        close: e.createCallbackNoArgs('onClose'),
                        buyBoxes: e.createCallbackNoArgs('onBuyBoxes'),
                        toggleAnimationState: e.createCallback(
                            (e) => ({ isAnimationActive: !e }),
                            'onAnimationStateChanged',
                        ),
                        setIsVideoPlaying: e.createCallback((e) => ({ isPlaying: e }), 'onVideoPlaying'),
                        setActiveRareReward: (0, G.aD)((e) => a.activeRareReward.set(e)),
                        clearActiveRareReward: (0, G.aD)(() => a.activeRareReward.set(null)),
                    }),
                ),
                L = z[0],
                X = z[1];
            var q = t(6483),
                M = t.n(q),
                Q = t(8844),
                U = t(9766),
                Y = t(8089),
                F = t(7522),
                J = t(5959),
                ee = t(5415),
                ae = t(8821),
                te = t(6974),
                re = t(3703),
                ne = t(2369),
                se = t(2862),
                ie = t(2056),
                oe = t(514),
                ce = t(7060),
                le = t(3633),
                me = t(4401),
                de = t(7039),
                pe = t(46);
            const ue = { opacity: 1, transform: 'translateX(0rem)', filter: 'brightness(1)' },
                _e = { opacity: 1, transform: 'scale(1)', filter: 'brightness(1)' },
                ge = { opacity: 0, transform: 'scale(1) rotateZ(0deg)', filter: 'brightness(1)' },
                we = () => ({ from: { opacity: 0, transform: 'translateX(-10rem)', filter: 'brightness(1)' } }),
                be = () => ({
                    to: [
                        { opacity: 1, transform: 'translateX(-8rem)', filter: 'brightness(1.5)' },
                        { opacity: 1, transform: 'translateX(-6rem)', filter: 'brightness(1.5)' },
                        { opacity: 1, transform: 'translateX(-4rem)', filter: 'brightness(1.3)' },
                        { opacity: 1, transform: 'translateX(-2rem)', filter: 'brightness(1.1)' },
                        ue,
                    ],
                    config: { duration: 30, easing: F.Z.easeOutCubic },
                }),
                ye = () => ({ from: { opacity: 0, transform: 'scale(1)', filter: 'brightness(1.5)' } }),
                ve = (e = 400) => ({
                    to: [{ opacity: 1, transform: 'scale(1.06)', filter: 'brightness(1.5)' }, _e],
                    config: { duration: e, easing: F.Z.easeOutCubic },
                }),
                fe = () => ({ from: { opacity: 1, filter: 'brightness(1)', transform: 'scale(1)' } }),
                he = () => ({ from: { opacity: 0 } }),
                Re = () => ({
                    to: { opacity: 1, transform: 'translateX(0rem)', filter: 'brightness(1)' },
                    config: { duration: 200, easing: F.Z.easeOutCubic },
                });
            var Ee = t(114),
                Ae = t(9108);
            const Ce = {
                    base: 'RewardTitle_base_e1',
                    base__premium_plus: 'RewardTitle_base__premium_plus_44',
                    value: 'RewardTitle_value_b5',
                    base__credits: 'RewardTitle_base__credits_f6',
                    base__gold: 'RewardTitle_base__gold_19',
                    base__freeXP: 'RewardTitle_base__freeXP_ec',
                    base__bptaler: 'RewardTitle_base__bptaler_f2',
                    base__crystal: 'RewardTitle_base__crystal_75',
                    pluralText: 'RewardTitle_pluralText_11',
                },
                xe = (0, m.Pi)(({ reward: e, isCompensation: a = !1, className: t = '' }) => {
                    const r = X().model,
                        s = ((e, a) => {
                            const t = e.name,
                                r = e.value,
                                n = e.compensation;
                            return a ? { name: n.name, value: n.value } : { name: t, value: r };
                        })(e, a),
                        i = s.name,
                        o = s.value,
                        c = r.computes.dynamicResources().dynamicTexts,
                        l = i === se.E4.PremiumPlus;
                    return n().createElement(
                        'div',
                        { className: M()(Ce.base, Ce[`base__${i}`], t) },
                        n().createElement(
                            'div',
                            { className: Ce.value },
                            n().createElement(U.z, { text: (0, me.QH)(o) }),
                        ),
                        l &&
                            n().createElement(
                                'span',
                                { className: Ce.pluralText },
                                c.rewardsPremiumDay.plural('premiumDay', Number(o.split(' ').at(-1))),
                            ),
                    );
                }),
                Ne = 'Compensation_base_ca',
                ke = 'Compensation_iconWrapper_66',
                Oe = 'Compensation_icon_69',
                Te = (0, m.Pi)(({ reward: e, size: a, icon: t, animationName: s, className: i = '' }) => {
                    const o = X().model.computes.resources().sounds,
                        c = (0, d.useSpring)(ye),
                        l = c[0],
                        m = c[1],
                        p = (0, g.H)(R);
                    return (
                        (0, r.useEffect)(() => {
                            const e = (e) => {
                                e.name === s && (oe.hY.sound(o.compensationAppear), m.start(ve()));
                            };
                            return (
                                p.events.on('change', e),
                                () => {
                                    p.events.off('change', e);
                                }
                            );
                        }),
                        (0, r.useEffect)(() => {
                            const e = () => {
                                m.start(ve(0));
                            };
                            return (
                                p.events.on('skipAll', e),
                                () => {
                                    p.events.off('skipAll', e);
                                }
                            );
                        }),
                        n().createElement(
                            'div',
                            { className: M()(Ne, i) },
                            n().createElement(
                                d.animated.div,
                                { style: Object.assign({}, l) },
                                n().createElement(Ae._, {
                                    key: 'compensation_image',
                                    icon: (0, Ee.ry)(e, se.h2.Big, e.isCompensation),
                                    sizes: { height: a.rewardHeight, width: a.rewardWidth },
                                }),
                            ),
                            n().createElement(
                                d.animated.div,
                                { style: { opacity: l.opacity }, className: ke },
                                n().createElement(Ae._, {
                                    key: 'compensation_icon',
                                    icon: t,
                                    sizes: { height: a.compensationIconHeight, width: a.compensationIconWidth },
                                    className: Oe,
                                }),
                            ),
                            n().createElement(
                                d.animated.div,
                                { style: { opacity: l.opacity } },
                                n().createElement(xe, { reward: e, isCompensation: e.isCompensation }),
                            ),
                        )
                    );
                }),
                Ie = ({ reward: e, className: a = '' }) => {
                    const t = e.vehicleShortName;
                    return n().createElement('div', { className: M()(Ce.base, a) }, t);
                },
                Be = {
                    base: 'AnimatedReward_base_c3',
                    container: 'AnimatedReward_container_c6',
                    reward: 'AnimatedReward_reward_8e',
                    video: 'AnimatedReward_video_3c',
                    glow: 'AnimatedReward_glow_7e',
                    compensation: 'AnimatedReward_compensation_c4',
                    title: 'AnimatedReward_title_3d',
                    vehicleTitle: 'AnimatedReward_vehicleTitle_08',
                    loupeWrapper: 'AnimatedReward_loupeWrapper_a1',
                    loupe: 'AnimatedReward_loupe_22',
                },
                Pe = [se.E4.Vehicles, se.E4.TmanToken],
                Se = {
                    rewardHeight: '80rem',
                    rewardWidth: '80rem',
                    countHeight: '18rem',
                    compensationIconWidth: '24rem',
                    compensationIconHeight: '24rem',
                    nameHeight: '20rem',
                    descriptionHeight: '20rem',
                    imageSize: se.h2.Big,
                    premDaysHeight: '65rem',
                    premDaysWidth: '65rem',
                },
                Ve = { opacity: 0, display: 'none' },
                De = { opacity: 1, display: 'flex' },
                He = { to: De, config: { duration: 500, easing: F.Z.easeOutCubic } },
                Ge = (0, m.Pi)(({ reward: e, rewardIndex: a, rowIndex: t }) => {
                    const s = X(),
                        o = s.model,
                        c = s.controls,
                        l = o.computes.resources(),
                        m = l.images,
                        p = l.texts,
                        _ = l.videos,
                        w = l.sounds,
                        b = o.info.eventName.get(),
                        y = (0, r.useState)(!1),
                        v = y[0],
                        E = y[1],
                        T = e.tooltipContentId,
                        I = e.tooltipId,
                        B = (0, i.v)(u.Kg, (e) => e.value),
                        S = (0, d.useSpring)((0, h.y)(e) ? he : we),
                        D = S[0],
                        H = S[1],
                        G = (0, d.useSpring)(fe),
                        W = G[0],
                        Z = G[1],
                        $ = (0, d.useSpring)(() => ({ from: Ve })),
                        K = $[0],
                        j = $[1],
                        z = (0, g.H)(R);
                    return (
                        (0, r.useEffect)(() => {
                            const r = (r) => {
                                switch (r.name) {
                                    case P(A, t).name:
                                        (0, h.y)(e) || 0 !== a || (H.start(be()), oe.hY.sound(w.multiRewardAppear));
                                        break;
                                    case P(C, t, a).name:
                                        (H.start((0, h.y)(e) ? be() : Re()), oe.hY.sound(w.multiRewardAppear));
                                        break;
                                    case P(N, t, a).name:
                                        E(!0);
                                        break;
                                    case P(x, t, a).name:
                                        c.setActiveRareReward(e);
                                        break;
                                    case P(O, t, a).name:
                                        Z.start({
                                            to: [
                                                {
                                                    opacity: 1,
                                                    transform: 'scale(1) rotateZ(0.25deg)',
                                                    filter: 'brightness(1.07)',
                                                },
                                                {
                                                    opacity: 1,
                                                    transform: 'scale(1.01) rotateZ(-0.5deg)',
                                                    filter: 'brightness(1.14)',
                                                },
                                                {
                                                    opacity: 1,
                                                    transform: 'scale(1.02) rotateZ(0.75deg)',
                                                    filter: 'brightness(1.21)',
                                                },
                                                {
                                                    opacity: 1,
                                                    transform: 'scale(1.03) rotateZ(-0.75deg)',
                                                    filter: 'brightness(1.28)',
                                                },
                                                {
                                                    opacity: 1,
                                                    transform: 'scale(1.04) rotateZ(1deg)',
                                                    filter: 'brightness(1.35)',
                                                },
                                                {
                                                    opacity: 1,
                                                    transform: 'scale(1.05) rotateZ(-1deg)',
                                                    filter: 'brightness(1.42)',
                                                },
                                                {
                                                    opacity: 1,
                                                    transform: 'scale(1.06) rotateZ(0deg)',
                                                    filter: 'brightness(1.5)',
                                                },
                                                ge,
                                            ],
                                            config: { duration: 125, easing: F.Z.easeInOutCubic },
                                        });
                                }
                            };
                            return (
                                z.events.on('change', r),
                                () => {
                                    z.events.off('change', r);
                                }
                            );
                        }, [z.events, t, a, e, H, c, Z, w]),
                        (0, r.useEffect)(() => {
                            const a = () => {
                                (H.start(
                                    (0, h.y)(e) ? Re() : { to: ue, config: { duration: 0, easing: F.Z.easeOutCubic } },
                                ),
                                    e.isCompensation &&
                                        Z.start({ to: ge, config: { duration: 0, easing: F.Z.easeInOutCubic } }));
                            };
                            return (
                                z.events.on('skipAll', a),
                                () => {
                                    z.events.off('skipAll', a);
                                }
                            );
                        }),
                        (0, r.useEffect)(() => {
                            switch (B) {
                                case u.B1.skip:
                                    j.set(De);
                                    break;
                                case u.B1.preparation:
                                    j.set(Ve);
                                    break;
                                case u.B1.page:
                                    j.start(He);
                            }
                        }, [B]),
                        n().createElement(
                            ie.u,
                            {
                                ignoreShowDelay: !0,
                                contentId: Number(T),
                                args: { tooltipId: I, boxCategory: e.icon, eventName: b },
                                isEnabled: B === u.B1.initial,
                            },
                            n().createElement(
                                'div',
                                { className: M()(Be.base, Be[`base__reward_${t}_${a}`]) },
                                n().createElement(
                                    d.animated.div,
                                    { style: Object.assign({}, D), className: Be.container },
                                    (0, h.y)(e) &&
                                        n().createElement(ce.o, {
                                            className: M()(Be.video, Be.glow),
                                            src: _.rareGlow,
                                            autoplay: !0,
                                            loop: !0,
                                        }),
                                    n().createElement(
                                        d.animated.div,
                                        { style: Object.assign({}, W) },
                                        n().createElement(pe.Q, {
                                            reward: e,
                                            countText: p.multiplier,
                                            sizes: Se,
                                            className: Be.reward,
                                            currentAnimationState: B,
                                        }),
                                    ),
                                    n().createElement(
                                        d.animated.div,
                                        { style: { opacity: W.opacity }, className: Be.title },
                                        e.name === se.E4.Vehicles &&
                                            n().createElement(Ie, { reward: e, className: Be.vehicleTitle }),
                                        !Pe.includes(e.name) && n().createElement(xe, { reward: e }),
                                    ),
                                    (0, me.XM)(e) &&
                                        !e.isCompensation &&
                                        n().createElement(
                                            d.animated.div,
                                            { style: K, className: Be.loupeWrapper },
                                            n().createElement(le.k, {
                                                icon: { img: m.previewIcon },
                                                onClick: () =>
                                                    c.goPreview({
                                                        bonusType: e.name,
                                                        bonusId: e.id ? e.id : '',
                                                        styleID: e.styleID,
                                                    }),
                                                className: Be.loupe,
                                            }),
                                        ),
                                    e.isCompensation &&
                                        n().createElement(Te, {
                                            reward: e,
                                            size: Se,
                                            icon: m.compensationIcon,
                                            animationName: P(k, t, a).name,
                                            className: Be.compensation,
                                        }),
                                ),
                                (0, h.y)(e) &&
                                    !(0, f.C)(b, e.specialAwardName) &&
                                    n().createElement(de.s, {
                                        className: Be.video,
                                        sound: e.rarity === V.G.Rare ? w.rareAnimation : w.epicAnimation,
                                        src: _[e.rarity],
                                        show: v,
                                        timer: { remainder: 0.5, next: z.resume },
                                    }),
                            ),
                        )
                    );
                }),
                We = {
                    base: 'RewardsRow_base_72',
                    number: 'RewardsRow_number_8e',
                    number__0: 'RewardsRow_number__0_d2',
                    number__1: 'RewardsRow_number__1_1b',
                    number__2: 'RewardsRow_number__2_ed',
                    number__3: 'RewardsRow_number__3_66',
                    number__4: 'RewardsRow_number__4_8d',
                    divider: 'RewardsRow_divider_36',
                    divider__0: 'RewardsRow_divider__0_78',
                    divider__1: 'RewardsRow_divider__1_4e',
                    divider__2: 'RewardsRow_divider__2_6b',
                    divider__3: 'RewardsRow_divider__3_3e',
                },
                Ze = (0, m.Pi)(({ order: e, rewards: a, isLast: t }) => {
                    const r = X().model.computes.maxRewardsInLine();
                    return n().createElement(
                        'div',
                        { className: We.base, style: { width: ((s = r), 28 + 108 * s) + 'rem' } },
                        n().createElement('div', { className: M()(We.number, We['number__' + (e - 1)]) }, e),
                        v.UI(a, (a, t) =>
                            n().createElement(Ge, {
                                reward: a,
                                key: `reward_${e - 1}_${t}_${(0, h.u)()}`,
                                rewardIndex: t,
                                rowIndex: e - 1,
                            }),
                        ),
                        !t && n().createElement('div', { className: M()(We.divider, We['divider__' + (e - 1)]) }),
                    );
                    var s;
                }),
                $e = 'Content_base_48',
                Ke = 'Content_base__ready_a2',
                je = 'Content_rareOverlay_2f',
                ze = 'Content_headerBlock_4d',
                Le = 'Content_subtitle_9e',
                Xe = 'Content_closeButton_15',
                qe = 'Content_boxes_69',
                Me = 'Content_footer_04',
                Qe = 'Content_checkbox_8f',
                Ue = { opacity: 0 },
                Ye = { opacity: 1 },
                Fe = {
                    to: Ye,
                    config: { duration: 500, easing: F.Z.easeOutCubic },
                    onRest: () => {
                        u.Kg.send({ type: u.QK.toInitial });
                    },
                },
                Je = (0, m.Pi)(() => {
                    const e = X(),
                        a = e.model,
                        t = e.controls,
                        s = a.root.get(),
                        o = s.boxesCount,
                        c = s.boxCategory,
                        m = s.isWindowAccessible,
                        p = s.isShopVisible,
                        _ = a.info.isAnimationActive.get(),
                        w = a.info.eventName.get(),
                        b = a.info.openingCount.get(),
                        y = a.activeRareReward.get(),
                        h = a.computes,
                        E = h.resources,
                        A = h.dynamicResources,
                        C = h.rewardsListByBoxes,
                        x = E(),
                        N = x.texts,
                        k = x.images,
                        O = x.sounds,
                        T = A().dynamicTexts,
                        I = C(),
                        B = (0, ee.GS)().mediaSize,
                        P = (0, i.v)(u.Kg, (e) => e.value),
                        S = (0, d.useSpring)(() => ({ from: Ue })),
                        V = S[0],
                        D = S[1],
                        H = (0, g.H)(R);
                    (0, r.useEffect)(() => {
                        switch ((0, l.P)(P)) {
                            case u.B1.skip:
                                (D.set(Ye), H.skipAll());
                                break;
                            case u.B1.preparation:
                                (D.set(Ue), H.reset());
                                break;
                            case u.B1.rewards:
                                H.start();
                                break;
                            case u.B1.page:
                                D.start(Fe);
                        }
                    }, [P]);
                    return n().createElement(
                        'div',
                        { className: M()($e, P === u.B1.initial && Ke) },
                        Boolean(y && (0, f.C)(w, null == y ? void 0 : y.specialAwardName)) &&
                            n().createElement(
                                'div',
                                { className: je },
                                n().createElement(ne.H, {
                                    res: (0, f.D)(w, null == y ? void 0 : y.specialAwardName),
                                    rareBonus: y,
                                    texts: N,
                                    controls: {
                                        onClose: () => {
                                            (t.setActiveRareReward(null), H.resume());
                                        },
                                        onPlay: () => t.setIsVideoPlaying(!0),
                                        onEnded: () => t.setIsVideoPlaying(!1),
                                    },
                                    minimized: !m,
                                }),
                            ),
                        n().createElement(
                            d.animated.div,
                            { style: V, className: ze },
                            n().createElement(te.h, {
                                title: N.headerTitle,
                                subTitle: c
                                    ? n().createElement(U.z, {
                                          text: N.headerSubtitle,
                                          binding: { name: T.boxCategory.dynOpt(c), boxesCount: I.length },
                                          classMix: Le,
                                      })
                                    : '',
                            }),
                            n().createElement(
                                'div',
                                { className: Xe },
                                n().createElement(Y.A, {
                                    caption: N.closeButton,
                                    type: 'close',
                                    side: 'right',
                                    onClick: t.close,
                                }),
                            ),
                        ),
                        n().createElement(
                            'div',
                            { className: qe },
                            v.UI(I, (e, a) =>
                                n().createElement(Ze, {
                                    key: `rewards_row_${a}`,
                                    order: a + 1,
                                    isLast: a === I.length - 1,
                                    rewards: e,
                                }),
                            ),
                        ),
                        n().createElement(
                            d.animated.div,
                            { style: V, className: Me },
                            J.graphicsQuality.isHigh() &&
                                n().createElement(ae.o, {
                                    isActive: _,
                                    onClick: () => {
                                        t.toggleAnimationState(_);
                                    },
                                    className: Qe,
                                    text: N.checkbox,
                                    size: B >= ee.cJ.Medium ? Q.yB.extraLarge : Q.yB.large,
                                }),
                            n().createElement(re.Z, {
                                texts: N,
                                images: k,
                                sounds: O,
                                actions: Object.assign({}, t, {
                                    openNext: () => {
                                        u.Kg.send({ type: u.QK.toPreparation });
                                    },
                                }),
                                boxesCount: o,
                                openingCount: b,
                                isShopVisible: p,
                            }),
                        ),
                    );
                }),
                ea = 'App_base_79',
                aa = 'App_background_02',
                ta = 'App_loader_4a',
                ra = 'App_content_62',
                na = 'App_loaderWrapper_e5',
                sa = (0, m.Pi)(() => {
                    const e = X(),
                        a = e.model,
                        t = e.controls,
                        s = a.computes.resources(),
                        m = s.images,
                        v = s.texts,
                        f = a.root.get().isWindowAccessible,
                        h = a.info.isAnimationActive.get(),
                        E = a.info.isAwaitingResponse.get(),
                        A = a.computes.hasRareReward(),
                        C = a.info.isReopen.get(),
                        x = a.computes.multimediaResource(),
                        N = (0, i.v)(u.Kg, (e) => e.value),
                        k = (0, g.H)(R);
                    ((0, r.useEffect)(() => {
                        const e = (e) => {
                            e.name === T && u.Kg.send({ type: u.QK.toPage });
                        };
                        return (
                            k.events.on('change', e),
                            () => {
                                k.events.off('change', e);
                            }
                        );
                    }, [k.events]),
                        (0, o.gd)(
                            c.n.ESCAPE,
                            () => {
                                N === u.B1.initial && t.close();
                            },
                            !0,
                        ),
                        (0, r.useEffect)(
                            () => (
                                u.Kg.start(),
                                () => {
                                    u.Kg.stop();
                                }
                            ),
                            [],
                        ),
                        (0, r.useEffect)(() => {
                            u.Kg.send({ type: u.QK.setAnimationActive, isAnimationActive: h });
                        }, [h]),
                        (0, b.s)(C, E, N));
                    const O = (0, w.H)(N, t.openNext),
                        I = O.loadingStyle,
                        B = O.contentStyle;
                    return n().createElement(
                        'div',
                        { className: ea, ref: k.rootRef },
                        n().createElement(_.F, {
                            activeType: A ? y.e.rare : y.e.common,
                            minimized: !f,
                            res: x,
                            className: aa,
                            onPlay: () => t.setIsVideoPlaying(!0),
                            onEnded: () => t.setIsVideoPlaying(!1),
                        }),
                        (0, l.P)(N) !== u.A_ &&
                            n().createElement(d.animated.div, { style: B, className: ra }, n().createElement(Je, null)),
                        N === u.B1.waiting &&
                            n().createElement(
                                d.animated.div,
                                { style: I, className: na },
                                n().createElement(p.a, { text: v.loader, img: m.loader, className: ta }),
                            ),
                    );
                });
            var ia = t(7051);
            const oa = (0, m.Pi)(({ children: e }) => {
                    const a = X().model,
                        t = a.info.eventName.get(),
                        r = (0, ia.G)({ steps: S(a.computes.rewardsListByBoxes(), t), autoStart: !1 })();
                    return n().createElement(R.Provider, { value: r }, e);
                }),
                ca = () =>
                    n().createElement(
                        L,
                        { options: s.f.MULTIPLE_REWARD },
                        n().createElement(oa, null, n().createElement(sa, null)),
                    );
        },
    },
]);
